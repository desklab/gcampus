...
def generator(queryset, parameter_type):
    ...
    for m in values:
        ...
        if current is None:
            ...
            current.update({pk: "" for pk in parameter_type})
...
def filter_csv_download(request):
    ...
    parameter_types = (
        ParameterType.objects.values_list("name", flat=True).order_by("pk").all()
    )
    ...
    writer = csv.DictWriter(
        buffer_, delimiter=",", quoting=csv.QUOTE_ALL, fieldnames=header
    )
    ...
    return StreamingHttpResponse(
        stream(generate_csv, writer, buffer_),
